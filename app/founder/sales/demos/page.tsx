...
export default async function DemosCalendarPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  const bookingHref = (id: string) => videoLink || `/founder/sales/${id}`;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link href="/founder/sales" className="text-sm text-white/70 hover:text-white">← Sales</Link>
          <h1 className="mt-1 text-2xl font-semibold text-white drop-shadow-sm">Demos</h1>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/founder/sales/demos?month=${prevM}`} className="grid h-8 w-8 place-items-center rounded-lg border border-white/40 bg-white/20 text-white hover:bg-white/30"><ChevronLeft className="h-4 w-4" /></Link>
          <span className="min-w-[150px] text-center text-sm font-semibold text-white">{MONTHS[mo - 1]} {yr}</span>
          <Link href={`/founder/sales/demos?month=${nextM}`} className="grid h-8 w-8 place-items-center rounded-lg border border-white/40 bg-white/20 text-white hover:bg-white/30"><ChevronRight className="h-4 w-4" /></Link>
        </div>
      </div>
              ...
              {upcoming.map((d) => {
                ...
                return (
                  <li key={d.id} className="flex flex-wrap items-center justify-between gap-3 py-2.5 text-sm">
                    <Link href={`/founder/sales/${d.id}`} className="font-medium text-gray-900 hover:text-brand-700 hover:underline">{d.name}</Link>
