...
import { Mail, MessageSquare, StickyNote, Send } from "lucide-react";
import {
  getApplicantThread,
  sendMessage,
  addNote,
...
export function ApplicantComms({
  ...
  return (
    <div>
      <p className="text-xs uppercase tracking-wide text-gray-400">Communication</p>
      ...
      <div className="mt-2 space-y-2 rounded-lg border border-gray-200 p-3">
        ...
        {channel !== "note" && channelTemplates.length > 0 && (
          <select defaultValue="" onChange={(e) => applyTemplate(e.target.value)} className={cls}>
        ...
        {channel !== "note" && (
          <p className="text-[11px] text-gray-400">
            Merge fields like {"{{first_name}}"} are filled in automatically when sent.
          </p>
        )}
      </div>
