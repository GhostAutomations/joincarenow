...
const NOUN_PLURAL: Record<Kind, string> = {
 ...
 *  Create button makes a new blank store template. Mirrors FounderStoreBrowser. */
export function FounderDocStoreBrowser({ kind, docs }: { kind: Kind; docs: FounderDocCard[] }) {
  ...
  function deleteSelected() {
    ...
    startTransition(async () => {
      await deleteStoreDocsBulk(kind, ids);
      setSelected(new Set());
      router.refresh();
    });
  }
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {filtered.length === 0 ? (
        <p className="mt-10 text-center text-sm text-gray-500">
          {docs.length === 0
            ? `No store ${NOUN_PLURAL[kind]} yet — create your first one.`
          ...
          {filtered.map((d) => (
            <div
              ...
              className={`group relative flex gap-4 rounded-2xl border bg-white/70 p-4 shadow-sm backdrop-blur-md transition hover:shadow-md ${
                selected.has(d.id) ? "border-brand-400 ring-1 ring-brand-300" : "border-white/50 hover:border-brand-200"
              }`}
            >
              <input
                type="checkbox"
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 pr-6">
                  <h3 className="truncate font-semibold text-gray-900">{d.name || "Untitled"}</h3>
                  <PriceBadge pricePence={d.price_pence} />
                  {d.published ? (
