  ...
  return (
    <div className="flex h-[70vh] flex-col rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-100 px-4 py-3">
        ...
        <p className="text-xs text-gray-400">Internal — not visible to applicants</p>
      </div>

      <div ref={scrollRef} className="flex-1 space-y-2 overflow-y-auto bg-gray-50 p-4">
        {messages.length === 0 && <p className="mt-8 text-center text-sm text-gray-400">No messages yet — say hello.</p>}
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.mine ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[78%] rounded-2xl px-3.5 py-2 text-sm shadow-sm ${m.mine ? "rounded-br-sm bg-brand-600 text-white" : "rounded-bl-sm border border-gray-200 bg-white text-gray-800"}`}>
              {m.applicant && (
                <p className={`mb-0.5 text-[10px] font-semibold ${m.mine ? "text-white/80" : "text-brand-600"}`}>Re: {m.applicant}</p>
              )}
      ...
      <form ref={formRef} action={action} className="space-y-2 border-t border-gray-100 p-3">
        <input type="hidden" name="recipientId" value={recipientId} />
        <div className="flex items-end gap-2">
          <select name="applicationId" defaultValue="" className="w-40 shrink-0 rounded-lg border border-gray-300 px-2 py-2 text-xs text-gray-600 focus:border-brand-500 focus:outline-none">
        ...
        <p className="text-[11px] text-gray-400">Tag an applicant to add this to their audit trail (staff-only).</p>
