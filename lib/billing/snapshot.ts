...
export async function recordBillingSnapshot(): Promise<void> {
  ...
  const { data } = await db
    .from("companies")
    .select("billing_status, billing_interval, commitment_until, extra_branches, billing_comped");
  ...
  const mrr = rows.reduce((s, r) => (isPaying(r) ? s + (r.billing_interval === "year" ? 550 / 12 : 55) + (r.extra_branches ?? 0) * 7.5 : s), 0);

  const snapshot = {
    snapshot_date: new Date().toISOString().slice(0, 10),
    mrr: Math.round(mrr * 100) / 100,
    paying: rows.filter(isPaying).length,
    monthly: rows.filter((r) => isPaying(r) && r.billing_interval !== "year" && !committed(r)).length,
