-- ============================================================
-- JOIN CARE NOW — Migration 0038: "Due within N days" workflow tasks
-- Templates store a relative due window (due_days). When a task fires at its
-- trigger point, the per-applicant task's due_date is computed as
-- today + due_days. apply_to_job now delegates on_application tasks to
-- create_stage_tasks so the logic lives in one place.
-- Run AFTER 0037_question_templates.sql.
-- ============================================================
...
-- apply_to_job: same as before, but delegates on_application tasks to
-- create_stage_tasks (honours due_days). SECURITY DEFINER runs as owner, so
-- the membership check inside create_stage_tasks is satisfied.
...
begin
  ...
  -- Fire any "on application" workflow tasks (honours due_days).
  perform public.create_stage_tasks(v_application_id, 'on_application');
