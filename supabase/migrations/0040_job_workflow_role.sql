-- ============================================================
-- JOIN CARE NOW — Migration 0040: Job workflow selector
-- A job advert can choose which role's workflow drives its tasks. Defaults to
-- the job's own role (set in the UI) but can be changed. create_stage_tasks
-- matches on workflow_role_id, falling back to the job's role_id.
-- Run AFTER 0039_workflow_role_association.sql.
-- ============================================================

alter table public.jobs
  add column if not exists workflow_role_id uuid references public.roles (id) on delete set null;
