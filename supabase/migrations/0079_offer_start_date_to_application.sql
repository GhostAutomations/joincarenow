-- ============================================================
-- JOIN CARE NOW — Migration 0079: carry the offer start date to the application
-- The offer captures a start date, but accepting never wrote it to the
-- application — so the employee record (which reads applications.start_date)
-- showed a blank Start date. On accept, copy the offer's start date onto the
-- application (offer wins if set). Covers both the simple accept and the
-- sign-and-accept paths. Run AFTER 0078_employees_delete_policy.sql.
-- ============================================================

-- Simple accept / decline (no documents to sign).
...
declare
  ...
  v_start_date date; v_reason text;
begin
  if p_response not in ('accepted', 'declined') then raise exception 'Invalid response'; end if;
  select o.id, o.company_id, o.application_id, a.applicant_id, o.status, o.start_date
    into v_id, v_company_id, v_application_id, v_applicant_id, v_status, v_start_date
...
declare
  v_id uuid; v_company_id uuid; v_application_id uuid; v_applicant_id uuid; v_status text;
  v_start_date date; v_doc jsonb;
begin
  if coalesce(btrim(p_signer_name), '') = '' then raise exception 'A signature is required'; end if;

  select o.id, o.company_id, o.application_id, o.applicant_id, o.status, o.start_date
    into v_id, v_company_id, v_application_id, v_applicant_id, v_status, v_start_date
